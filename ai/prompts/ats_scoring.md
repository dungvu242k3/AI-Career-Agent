...
2. `experience_gap_analysis`: A clear 2–3 sentence assessment of the candidate's experience fit, highlighting strengths and specific areas to address in an interview or CV update.
3. `matched_skills` and `missing_skills`: Populated with `SkillMatchItem` objects detailing the match type, CV evidence (if any), JD requirement, and importance.
4. `excess_skills`: List of skills from the candidate's CV that are not requested in this JD (helps identify potential CV bloating).
